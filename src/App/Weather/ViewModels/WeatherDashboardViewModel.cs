...
using WhereToFly.App.Weather.Popups;
...
public class WeatherDashboardViewModel : ViewModelBase
{
    ...
    /// </summary>
    private readonly IWeatherDashboardIconDataService dataService;
    ...
    {
        this.dataService = Services.GetRequiredService<IWeatherDashboardIconDataService>();
    ...
    {
        await this.dataService.ClearList();
        await this.dataService.AddList(
    ...
    {
        this.weatherIconDescriptionList =
            (await this.dataService.GetList()).ToList();
    ...
    {
        var result =
            await MainPage.ShowPopupAsync<WeatherIconDescription>(
                new SelectWeatherIconPopupPage(),
                DefaultPopupOptions);

        WeatherIconDescription? weatherIcon = result?.Result;
    ...
    private async Task AddWebLinkAsync()
    {
        var result =
            await MainPage.ShowPopupAsync<WeatherIconDescription>(
                new AddWeatherLinkPopupPage(),
                DefaultPopupOptions);

        WeatherIconDescription? weatherIconDescription = result?.Result;
        ...
        await this.dataService.Add(weatherIconDescription);
